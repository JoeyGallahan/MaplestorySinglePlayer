...
public class PhysicsObject : MonoBehaviour
{
    ...
    public bool disableGroundedCheck = false;
    [SerializeField] public bool enableGravity = true;
    public bool itemMode = false;

    //Raycasting for collision
    BoxCollider2D physCollider;
    ...
    private void Awake()
    {
        physCollider = GetComponent<BoxCollider2D>();
    ...
    {
        if (velocity.y <= 0 && !disableGroundedCheck) //Don't want to check for bottom collisions if you're moving up
        {
            ...
            for (int i = 0; i < verticalRayCount; i++)
            {
                ...
                {                   
                    if (hit.collider != physCollider && hit.distance <= collisionRange) //If we hit something that's not itself and within the collision distance (using distance in the actual raycast isn't accurate so we just check it here)
                    {
